...
- **Жёсткий гейт пейринга (требование владельца: «всегда в pairing быть не
  должно») — на уровне Link Layer + SMP**:
  1. Вне 35-секундного окна (кнопка BOOT) connect-реклама фильтруется
     whitelist'ом контроллера, который строится из бондов в NVS
     (`ble_store_util_bonded_peers` → `ble_gap_wl_set`, filter policy
     `BLE_HCI_ADV_FILT_SCAN`). Посторонний не может даже инициировать
     соединение — SMP не начинается, зомби-бонды не возникают. Бонус: вне
     окна чужие сканеры не получают и scan response — для них устройство
     «безымянное» (визард columba показывает только именованные RNode).
  2. REPEAT_PAIRING (устройство удалило бонд только у себя и пытается
     перепейриться) вне окна молча игнорируется
     (`BLE_GAP_REPEAT_PAIRING_IGNORE`) — иначе любой когда-либо
     забонденный мог бы перепейриться в любой момент, что сводило бы гейт
     на нет. В окне — штатный путь: удалить бонд + снять resolving-запись
     + RETRY.
  3. **Единый бонд** (требование владельца): при успешном шифровании
     удаляются все бонды, кроме текущего пира (`purge_other_bonds`) —
     предыдущее устройство выпадает из whitelist и подключиться больше
     не может, пока не спейрится через новое окно.
  Забонденное устройство (RPA резолвится через resolving list по его IRK)
  подключается всегда без кнопки — это нормальное поведение.
